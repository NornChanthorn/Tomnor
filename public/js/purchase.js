$(function () {
  ...
  $('table#product-table tbody').on('change', 'input.quantity', function() {
    // var max_qty = parseFloat($(this).data('rule-max'));
    var entered_qty = $(this).val();

    var tr = $(this).parents('tr');

    var unit_price_inc_tax = tr.find('input.purchase-price').val();
    var line_total = entered_qty * unit_price_inc_tax;
    line_total = Number(line_total).toFixed(2);

    tr.find('input.sub-total').val(line_total);

    calculateTotal();
  });

  //Update line total and check for quantity not greater than max quantity
  $('table#product-table tbody').on('change', 'input.purchase-price', function() {
    // var max_qty = parseFloat($(this).data('rule-max'));
  ...
  //Update discount
  $('table#product-table tfoot').on('change', 'input.discount', function() {
    calculateDueBalance();
  });

  //Update shipping charges
  $('table#product-table tfoot').on('change', 'input.shipping-cost', function() {
    calculateDueBalance();
  });
});
  ...
  if (!isProductAdded) {
    let productRow =
      '<tr data-id="' + indexId + '">' +
        ...
        '<td>' + ((productElm.sub_sku == null || productElm.sub_sku == "") ? productElm.code : productElm.sub_sku || noneLabel) + '</td>' +
        '<td width="15%">' +
          '<input type="text" name="products[' + indexId + '][quantity]" value="1" class="form-control form-control-sm integer-input quantity" min="1" max="10000" required>' +
        '</td>' +
...
function calculateTotal() {
  ...
  $('table#product-table tbody tr').each(function() {
    ...
    let cost = $(this).find('input.purchase-price').val();
