$(function () {
  // When add product to sell list
  $('#add-product').click(function () {
      if ($('#product').attr('required', true).valid()) {
          let productId = $('#product').val();
          let isProductAdded = ($('#sell_return_table tbody').find('tr[data-id="' + productId + '"]').length > 0);
          ...
          if (!isProductAdded) {
              ...
              $('#sell_return_table tbody').append(productRow);
              formatNumericFields();
          }
  ...
  $('table#sell_return_table tbody').on('change', 'input.return_quantity', function() {
    // var max_qty = parseFloat($(this).data('rule-max'));
    var entered_qty = $(this).val();
    var tr = $(this).parents('tr');

    var unit_price_inc_tax = tr.find('input.sell_price').val();
  ...
  //Update line total and check for quantity not greater than max quantity
  $('table#sell_return_table tbody').on('change', 'input.sell-price', function() {
    // var max_qty = parseFloat($(this).data('rule-max'));
  ...
  //Update discount
  $('table#sell_return_table tfoot').on('change', 'input.discount', function() {
    calculateDueBalance();
  });

  //Update shipping charges
  $('table#sell_return_table tfoot').on('change', 'input.shipping-cost', function() {
    calculateDueBalance();
  });
});

// $(document).on('change', ".quantity, .sell-price", function() {
//   calculateTotal();
// });
...
function removeProduct(buttonElm) {
  $(buttonElm).parents('#sell_return_table tbody tr').remove();
...
function addProduct(productElm) {
  ...
  let isProductAdded = ($('#sell_return_table tbody').find('tr[data-id="' + indexId + '"]').length > 0);
    ...
    let productRow =
      '<tr data-id="' + indexId + '">' +
        ...
        '<td width="15%" class="text-right">' +
          '<input type="text" name="products[' + indexId + '][sell_price]" value="'+ productElm.cost+'" class="form-control form-control-sm decimal-input sell-price">' +
        '</td>' +
        '<td width="15%">' +
...
function calculateTotal() {
  ...
  $('table#sell_return_table tbody tr').each(function() {
    ...
    let cost = $(this).find('input.sell_price').val();
