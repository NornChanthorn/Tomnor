...
@section('title', trans('app.supplier'))

@section('content')
<main class="app-content">
  <div class="tile">
    <h3 class="page-heading">{{ trans('app.supplier') }}</h3>
      ...
      <table class="table table-hover table-bordered">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ @$supplier->province->khmer_name ?? '' }}</td>
            <td class="text-right">
              @include('partial/anchor-show', ['href' => route('contact.show', $supplier->id)])
              @if ($supplier->is_default == 0)
                @if(isAdmin() || Auth::user()->can('supplier.edit'))
                <a href="javascript::void(0);" class="btn btn-sm btn-primary mb-1 btn-modal" title="{{ trans('app.edit') }}" data-href="{{ route('contact.edit', $supplier->id) }}" data-container=".contact-modal">
                  <i class="fa fa-edit"></i>
                </a>
                @endif
                @if (isAdmin() || Auth::user()->can('supplier.delete'))
                  @include('partial/button-delete', ['url' => route('contact.destroy', $supplier->id)])
                @endif
              @endif
            </td>
          </tr>
...
@section('js')
  ...
  <script type="text/javascript">
    var contactExist = "{{ trans('message.supplier_already_exists') }}";
