...
<main class="app-content">
  <div class="tile">
    ...
    <div class="row">
      <div class="col-lg-6">
        @include('partial.anchor-create', ['href' => route('purchase.create')])
      </div>
      ...
      <table class="table table-bordered table-striped table-hover">
        <thead>
          <tr>
            ...
            <th class="text-center">@sortablelink('purchase_status', trans('app.status'))</th>
                  ...
                  <div class="btn-group" role="group">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                      @if (Auth::user()->can('po.browse'))
                        <a href="{{ route('purchase.invoice', $purchase->id) }}" title="{{ trans('app.invoice') }}" class="dropdown-item" target="_blank"><i class="fa fa-print"></i> {{ trans('app.invoice') }}</a>
                      @endif
              
                      <div class="dropdown-divider"></div>
                      @if(Auth::user()->can('po.browse'))
                        <a href="{{ route('purchase.show', $purchase->id) }}" class="dropdown-item" title="{{ __('app.view_detail') }}"><i class="fa fa-eye"></i> {{ __('app.view_detail') }}</a>
                      @endif

                      
                      @if(Auth::user()->can('po.edit') && $purchase->status!='final')
                        <a href="{{ route('purchase.edit', $purchase->id) }}" class="dropdown-item" title="{{ __('app.edit') }}"><i class="fa fa-pencil-square-o"></i> {{ __('app.edit') }}</a>
                      @endif
                      
                      {{-- @if(Auth::user()->can('purchase-return.add') && $purchase->status == 'received')
                        <a href="{{ route('purchase-return.add', $purchase->id) }}" class="dropdown-item" title="{{ __('app.purchase_return') }}"><i class="fa fa-reply"></i> {{ __('app.purchase_return') }}</a>
                      @endif --}}
                      @if(Auth::user()->can('po.delete'))
                        <a href="javascript:void(0);" title="{{ __('app.delete') }}" data-url="{{ route('purchase.destroy', $purchase->id) }}" class="dropdown-item btn-delete"><i class="fa fa-trash-o"></i> {{ __('app.delete') }}</a>
                      @endif
                      <div class="dropdown-divider"></div>
