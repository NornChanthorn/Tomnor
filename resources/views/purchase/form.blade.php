            ...
            <div class="input-group">
              <select name="supplier" id="supplier" class="form-control select2" required>
                @foreach ($suppliers as $index => $supplier)
                  <option value="{{ $supplier->id }}" {{ selectedOption($supplier->id, (old('supplier', $purchase->contact_id))) }} @if($supplier->is_default) selected @endif>
                    {{ $supplier->supplier_business_name }}, {{ $supplier->name }}
                  </option>
                @endforeach
              </select>
              <div class="input-group-append">
                <a href="#" class="btn btn-block btn-primary" id="add-supplier" data-href="{{ route('contact.create', ['type'=>$contact->type]) }}" data-container=".contact-modal"><i class="fa fa-plus"></i></a>
              </div>
            </div>
          </div>
                ...
                <tbody>
                  ...
                  @if($purchase->purchase_lines)
                    @foreach ($purchase->purchase_lines as $index => $purchase_line)
                      ...
                      @endphp
                      <tr data-id="{{ $indexId }}">
                        <input type="hidden" name="products[{{ $indexId }}][purchase_line_id]" value="{{ $purchase_line->id }}">
